...
func (c *Client) Post(ctx context.Context, req Request) Result {
	...
	httpReq.Header.Set("X-Adsb-Timestamp", fmt.Sprintf("%d", unix))
	httpReq.Header.Set("X-Adsb-Nonce", nonce)
	httpReq.Header.Set("X-Adsb-Signature", sig)
	httpReq.Header.Set("X-Adsb-Report-Id", req.ReportID)
	httpReq.Header.Set("X-Adsb-Forward-Id", req.ForwardID)
	httpReq.Header.Set("X-Adsb-Attempt", fmt.Sprintf("%d", req.Attempt))
	httpReq.Header.Set("X-Adsb-Radar", req.RadarID)
	httpReq.Header.Set("X-Adsb-Body-Sha256", hashutil.SHA256Hex(req.Body))

	resp, err := c.http.Do(httpReq)
	if err != nil {
		return Result{Error: fmt.Errorf("feeder post: %w", err)}
	}
