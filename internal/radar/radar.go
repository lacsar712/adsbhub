...
func (in CreateInput) normalize() (CreateInput, error) {
	...
	for _, p := range in.KindPrefixes {
		...
		if err := report.ValidateKind(strings.TrimSuffix(p, ".")); err != nil && !strings.HasSuffix(p, ".") {
			stem := strings.TrimSuffix(p, ".")
			if err2 := report.ValidateKind(stem); err2 != nil {
...
func (d Radar) Matches(kind string) bool {
	if !d.Enabled {
		return false
	}
	for _, p := range d.KindPrefixes {
		if report.MatchKind(kind, p) {
			return true
		}
	}
	return false
...
func (d Radar) OutboundSecrets(now time.Time) []string {
...
type Registry struct {
	mu    sync.Mutex
	clk   clock.Clock
	items map[string]Radar
}

func NewRegistry(clk clock.Clock) *Registry {
	return &Registry{clk: clk, items: make(map[string]Radar)}
}

func (r *Registry) Create(in CreateInput) (Radar, error) {
	in, err := in.normalize()
	if err != nil {
		return Radar{}, err
	}
...
func (r *Registry) Put(d Radar) error {
	if d.ID == "" {
		return fmt.Errorf("radar id required")
	}
...
func (r *Registry) Get(id string) (Radar, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
	return d, ok
...
func (r *Registry) SetEnabled(id string, enabled bool) (Radar, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
	if !ok {
		return Radar{}, fmt.Errorf("radar %s not found", id)
	}
	d.Enabled = enabled
	d.UpdatedAt = r.clk.Now()
	r.items[id] = d
...
func (r *Registry) RotateSecret(id, next string, overlap time.Duration) (Radar, error) {
	if len(next) < 8 {
		return Radar{}, fmt.Errorf("secret must be at least 8 characters")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
...
func (r *Registry) List() []Radar {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]Radar, 0, len(r.items))
...
func (r *Registry) Matching(kind string) []Radar {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]Radar, 0)
	for _, d := range r.items {
...
func (r *Registry) Restore(items []Radar) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.items = make(map[string]Radar, len(items))
	for _, d := range items {
		r.items[d.ID] = d
	}
}

func (r *Registry) Public(d Radar) map[string]any {
