...
import (
	...
	"github.com/lacsar712/adsbhub/internal/circuit"
...
func newTestEngine(t *testing.T, url string, clk clock.Clock) (*Engine, *radar.Radar) {
	t.Helper()
	return newTestEngineOpts(t, url, clk, engineOpts{})
}

func newTestEngineOpts(t *testing.T, url string, clk clock.Clock, opts engineOpts) (*Engine, *radar.Radar) {
	t.Helper()
	if opts.rate <= 0 {
		opts.rate = 100
	}
	...
	enabled := true
	d, err := radars.Create(radar.CreateInput{
		Name:         "t",
		URL:          url,
		Secret:       "abcdefgh",
		KindPrefixes: []string{""},
	...
	e := New(clk, broker, radars, gates, feeder.New(opts.timeout), journal.New(50), dlq.New(50), backoff.Policy{
...
func sampleJob(radarID string, clk clock.Clock, forwardID string) job.Job {
	return job.Job{
		ReportID:  "rpt_test",
		ForwardID: forwardID,
		RadarID:   radarID,
		Kind:      "adsb.position",
		Body:      []byte(`{"kind":"adsb.position","icao":"ABC123","lat":51.47,"lon":-0.45,"squawk":"7000","payload":{}}`),
		Attempt:   0,
		NotBefore: clk.Now(),
		CreatedAt: clk.Now(),
	}
...
func TestHandleRetriesStatus429(t *testing.T) {
	...
	j := sampleJob(dest.ID, clk, "fwd_test429")
...
func TestHandleTreats202AsSuccess(t *testing.T) {
	...
	clk := clock.NewFrozen(time.Unix(1_700_000_000, 0))
	e, dest := newTestEngine(t, srv.URL, clk)
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_test202"))
	if e.dead.Len() != 0 {
...
func TestHandleHonorsCancel(t *testing.T) {
	...
	go func() {
		defer close(done)
		e.handle(ctx, sampleJob(dest.ID, clk, "fwd_cancel"))
	}()
	select {
	case <-accepted:
...
func TestHandleTimeoutIsRetryable(t *testing.T) {
	...
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_timeout"))
	if e.dead.Len() != 0 {
...
func TestHandleSuccessClearsBreaker(t *testing.T) {
	...
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_brk_1"))
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_brk_2"))
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_brk_3"))
...
func TestHandleRespectsRateLimit(t *testing.T) {
	...
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_rl_1"))
	e.handle(context.Background(), sampleJob(dest.ID, clk, "fwd_rl_2"))
	if hits.Load() != 1 {
