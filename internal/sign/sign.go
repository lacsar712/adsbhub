...
var (
	...
	ErrBadScheme     = errors.New("signature scheme must be ads1")
...
func Sign(secret string, unixSec int64, nonce string, body []byte) (string, error) {
	...
	_, _ = mac.Write([]byte(hashutil.CanonicalADS1(unixSec, nonce, body)))
...
func Verify(clk clock.Clock, window time.Duration, secrets []string, hdr Headers, body []byte) error {
	...
	canonical := []byte(hashutil.CanonicalADS1(hdr.Timestamp, hdr.Nonce, body))
...
func HeaderMap(unixSec int64, nonce, signature string) map[string]string {
	return map[string]string{
		"X-Adsb-Timestamp": fmt.Sprintf("%d", unixSec),
		"X-Adsb-Nonce":     nonce,
		"X-Adsb-Signature": signature,
	}
}
